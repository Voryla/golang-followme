...
func stackalloc(n uint32) stack {
	...
	if n < _FixedStack<<_NumStackOrders && n < _StackCacheSize {
		...
		var x gclinkptr
		// 小栈分配，如果没有分配栈缓存空间，则进入 stackpoolalloc 函数进行空间分配(需要加锁)，这里最终是从
		// 全局的mheap也就是堆空间中获取内存空间
		if stackNoCache != 0 || thisg.m.p == 0 || thisg.m.preemptoff != "" {
			...
			// 尝试从g对应的mcache中的stackcache上获取内存空间(无锁)
			c := thisg.m.p.ptr().mcache
			x = c.stackcache[order].list
			// 若mcache中stackcache上没有足够的空间
			if x.ptr() == nil {
				// 调用stackcacherefill对stackpool进行扩容，（也就是从mheap中拿取，加锁），然后分配给协程
...
//调度发生的时机主要在执行函数调用阶段。函数调用是一个比较
//安全的检查点，Go语言编译器会在函数调用前判断stackguard0的大
//小，从而选择是否调用runtime.morestack_noctxt函数。
//morestack_noctxt为汇编函数，函数的执行流程如下：
...
func newstack() {
	...
	if preempt {
		...
		// 调用 gopreempt_m 函数，取消G与M之间的绑定关系，将G的状态转换为 _Grunnable 将G放入全局运行队列，并调用 schedule 函数开始下一轮调度循环
