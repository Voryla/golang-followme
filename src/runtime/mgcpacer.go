...
type gcControllerState struct {
	// 该值有GOGC环境变量设置，会影响GC的触发阈值：heapMinimum*GOGC/100
...
// 重置 GC 控制器状态，并以25%CPU利用率计算正式工和小时工数值
...
func (c *gcControllerState) startCycle() {
	...
	// 当GODEBUG=gcstoptheword 并发关闭，全部称为正式工
...
func (c *gcControllerState) findRunnableGCWorker(_p_ *p) *g {
	...
	// 消费工人的名额，设置 P.worker 工种
	if decIfPositive(&c.dedicatedMarkWorkersNeeded) {
		// This P is now dedicated to marking until the end of
		// the concurrent mark phase.
		// 设置当前 p 负责标记的标记工人工种
		_p_.gcMarkWorkerMode = gcMarkWorkerDedicatedMode
	} else if c.fractionalUtilizationGoal == 0 { // 如果当前工人名额已经消费完毕，即标记工作已经有足够的p执行了
	...
	gp := node.gp.ptr()
	// 唤醒标记工人,这里仅修改g的状态就行了，调度有GMP的schedule负责
