...
// 在堆上创建defer
// siz：表示参数+返回值共占用多大空间， fn：指向对应funcval的地址
//go:nosplit
...
func deferprocStack(d *_defer) {
	...
	// 注意， siz 和 fn 已经在编译阶段(ssa.go)完成设置，这里值初始化了其他字段
	...
	// 尽管栈上进行分配，仍然需要将多个 _defer 记录通过链表进行串联，
...
// 在堆上创建defer，期间可能使用调度器 _defer 结构缓存或者本地P _defer 缓存
//
//go:nosplit
func newdefer(siz int32) *_defer {
	var d *_defer
	// 根据 defer 传入时函数的参数总大小决定defer的等级
	sc := deferclass(uintptr(siz))
	gp := getg()
	// 检查 defer 参数的大小是否从 p 的 deferpool 直接分配
	if sc < uintptr(len(p{}.deferpool)) {
		pp := gp.m.p.ptr()
		// 如果 p 本地无法分配，则从全局池中获取该defer等级一半 defer缓存， 来填充 P 的本地资源池
		if len(pp.deferpool[sc]) == 0 && sched.deferpool[sc] != nil {
			// 出于性能考虑，如果从全局defer池获取一半defer时发生栈增长，则会调用 more stack
			// 将进一步降低 defer 的性能。因此切换到系统栈上执行，进而不会发生栈增长
...
func deferreturn() {
	...
	if d.openDefer {
		...
		gp._defer = d.link
		// 对 defer 进行回收，不过如果 _defer.heap = false 即在栈上的defer对象不需要回收
		freedefer(d)
		return
	}
...
func gopanic(e interface{}) {
	...
	// 因此可以通过 gp.m.curg == gp 来判断所在栈，gp.m.curg获取的是绑定在M的G(用户G)
	// 系统栈上的 panic 无法恢复
	if gp.m.curg != gp {
		print("panic: ")
		printany(e)
	...
	// 保存对应的消息
	p.arg = e
	// 指向保存在 goroutine 链表中先前的 panic 链表
	p.link = gp._panic
	gp._panic = (*_panic)(noescape(unsafe.Pointer(&p)))
