...
type mheap struct {
	...
	//
	// 两级管理已分配内存，L1[0]->L2[0]->heapArena(已分配内存)
...
// mheap_.arenaHints.
// 每个 areaHint 记录可分配的起始地址(累进)以及分配方向(向高位或低位)。如分配失败，则尝试
// 从链表获取下一个 arenaHint 重试，或有操作系统提供随机地址。
//
//go:notinheap
...
type mspan struct {
	...
	// out memory.
	allocBits  *gcBits // 分配标记位图，依据位图，将整个mspan内存当作object数组对待，通过索引计算偏移量
	...
	// 对象存活周期代龄
	// 不同于其他语言里描述对象存活周期的代龄概念，此代龄表示mspan清理状态
	sweepgen    uint32
...
const (
	// 数组(mcache.alloc)长度是 类别(sizeclass)的2倍
	// 每种类别分为scan和noscan两种，如此有助于GC优化
...
func (h *mheap) setSpans(base, npage uintptr, s *mspan) {
	p := base / pageSize
	// 计算 L1、L2 ，获取heapArena
	ai := arenaIndex(base)
	ha := h.arenas[ai.l1()][ai.l2()]
	// 按页填充一个或多个 heapArena.spans
...
func (h *mheap) allocSpan(npages uintptr, typ spanAllocType, spanclass spanClass) (s *mspan) {
	...
	if base == 0 {
		...
		// 失败，扩张后重试
		if base == 0 {
			// 获取基地址失败，说明heap中没有内存空间了，调用grow从操作系统中申请空间
			if !h.grow(npages) {
				unlock(&h.lock)
				return nil
			}
