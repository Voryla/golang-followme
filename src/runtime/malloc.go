...
//
// 如果 mspan.needzero 为 false，则 mspan 中的空闲对象槽已经清零。
// 否则，如果 needzero 为真，则在分配对象时将其清零。以这种方式延迟归零有多种好处：
...
//
//  更好的时间局部性，因为程序可能在清除后立即写入数据到mspan中
//	2. It exhibits better temporal locality, since the program is
//	   probably about to write to the memory.
//
//  虽然mspan.needzero位true(被清除的mspan中还有对象)，
//  但是mspan只要不被当作对象分配时使用，其中的对象就不会被清除
...
//
// 运行时管理的地址空间区域在任何给定时间可能处于以下四种状态之一
// Regions of the address space managed by the runtime may be in one of four
// states at any given time:
// 状态一： 未保留和未映射，任何区域的默认状态，任何区域的默认指向，二进制零区域
// 1) None - Unreserved and unmapped, the default state of any region.
// 状态二： 由运行时拥有(权限级别)(不一定拥有物理页)，访问其会导致错误，不计入进程的内存占用
// 2) Reserved - Owned by the runtime, but accessing it would cause a fault.
//               Does not count against the process' memory footprint.
// 状态三：保留的，未分配(没有对应物理页)，通过该状态的虚拟内存从页表中换进换出实现物理内存分配和回收，
// 同时弥补调用mmap时的权限位
...
//
// 系统调用：堆内存分配，内部实现根据OS平台调用不同的系统调用分配内存
// 同时调整地址空间区域的状态 None -> Ready
...
// 解锁一块地址空间    reserved -> prepared
// sysMap transitions a memory region from Reserved to Prepared. It ensures the
// memory region can be efficiently transitioned to Ready.
//
// 从保留的内存地址空间中获取一块地址空间，并将其转化为ready   prepared -> ready
...
func mallocinit() {
	...
	// 增加GOphysHugePageShift至 1<<physHugePageShift == physHugePageSize
	...
	if sys.PtrSize == 8 {
		...
		//
		// gccgo中的堆栈扫描仍然是保守的，因此地址与其他数据的区别很重要。
		// 表示，go的基本数据类型不包含特定的边界，因此Go无法将一个存放了地址的int区分内部
		// 是存放的int还是address
...
func (h *mheap) sysAlloc(n uintptr) (v unsafe.Pointer, size uintptr) {
	...
	// 用于 32-bit 分配。 heap.arena 已在 mallocinit 32-bit 块内初始化
	// 如果当前 64-bit,那么 h.arena.alloc 返回nil
	...
	for h.arenaHints != nil {
		...
		if p+n < p {
			...
			v = nil
		} else if arenaIndex(p+n-1) >= 1<<arenaBits { // 越界
	...
	if size == 0 {
		...
		// 将该地址左右两个区域存储到链表内，供下次使用
		// 右侧，高->低
		// Create new hints for extending this region.
		hint := (*arenaHint)(h.arenaHintAlloc.alloc())
		hint.addr, hint.down = uintptr(v), true
		// 将hint放在原链表前，将原链表指向新的链表首部，即hint
		hint.next, mheap_.arenaHints = mheap_.arenaHints, hint
		// 左侧， 低->高
		hint = (*arenaHint)(h.arenaHintAlloc.alloc())
 ...
 mapped:
	...
	// 根据地址范围，创建对应的 heapArean，以便后续保存元数据。
...
// 堆上分配的零长度(即便是不同类型的零长度对象)都指向同一个全局变量，以获得合法内存地址
...
func (c *mcache) nextFree(spc spanClass) (v gclinkptr, s *mspan, shouldhelpgc bool) {
	...
	if freeIndex == s.nelems {
		...
		// 从mcentral 获取新的 mspan 放入 alloc 数组
		c.refill(spc)
		shouldhelpgc = true
		s = c.alloc[spc]
		// 重试
...
// 在为对象分配内存后，mallocgc 函数会检查垃圾回收触发条件，并依照相关状态启动或参与辅助回收
...
func mallocgc(size uintptr, typ *_type, needzero bool) unsafe.Pointer {
	...
	if size <= maxSmallSize {
		// 判断是否为无须扫描非指针微小对象
		// 微小对象不能为指针，因为多个小对象被组合到一个object里，显然无法应对垃圾扫描。
		// 其次，它从span.freelist获取一个16字节的object，然后利用偏移量来记录下一次分配的位置。
		if noscan && size < maxTinySize {// 微小对象分配
			...
			// 获取新的tiny块
			// 就是从sizeclass=2的span.freelist获取一个16字节的object
			// Allocate a new maxTinySize block.
			span = c.alloc[tinySpanClass]
			v := nextFreeFast(span)
			// 若没有可用的object，那么需要从central获取新的span
			if v == 0 {
				// 重新提取tiny块
			...
			(*[2]uint64)(x)[1] = 0
			// 对比新旧两个tiny块的剩余空间
			// 新块分配之后，其tinyoffset=size，因此比对偏移量即可
