...
// 将根对象(协程栈，全局(bss未初始化全局变量，data已初始化全局变量))放入扫描队列中，，并初始化与扫描相关的状态
...
func gcMarkRootPrepare() {
	...
	//
	// We depend on addfinalizer to ma rk objects that get
...
func markroot(gcw *gcWork, i uint32) {
	// markrootBlock、scanstack 最终将调用 scanblock 函数
...
func gcAssistAlloc1(gp *g, scanWork int64) {
	...
	// 如果所有工人都在等待，且没有任务可用，表示整个标记任务结束
	if incnwait == work.nproc && !gcMarkWorkAvailable(nil) {
		// This has reached a background completion point. Set
		// gp.param to a non-nil value to indicate this. It
		// doesn't matter what we set it to (it just has to be
		// a valid pointer).
		gp.param = unsafe.Pointer(gp)//标记结束
	}
...
// GC 标记工作，从根对象开始，标记存活对象。内容尚未扫描的灰色对象，放入队列等待处理。
...
func gcDrain(gcw *gcWork, flags gcDrainFlags) {
	...
	// 公共账户，工人将劳动所得存入公共账号，为欠债休眠的G偿还债务，唤醒他们继续工作
