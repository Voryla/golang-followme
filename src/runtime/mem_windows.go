...
// sysUnused 通过系统调用告知操作系统某段内存暂不使用，建议内核回收对应物理内存。
// 当然，这只是一个建议，是否回收由内核决定。如物理内存资源充足，该建议可能会被忽略，以避免无谓的损耗。
// 而当再次使用该内存块时，会引发缺页异常，内核会自动重新关联物理内存页
...
// linux系统此函数内部调用 sysHugePage 完成内存分配
// windows系统内存分配在此函数中完成
...
func sysReserve(v unsafe.Pointer, n uintptr) unsafe.Pointer {
	...
	// 函数mmap要求操作系统内核从创建新的虚拟存储器区域，可指定起始地址和长度。Windows没有此函数，对应API是_VirtualAlloc
