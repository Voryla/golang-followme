...
TEXT _rt0_amd64(SB),NOSPLIT,$-8
	MOVQ	0(SP), DI	// argc
	LEAQ	8(SP), SI	// argv
	JMP	runtime·rt0_go(SB) // runtime/asm_amd64.s  line 81
...
ok:
	...
	// os 初始化  确定了两个很重要的运行时常量，即处理器核心数以及内存物理页大小 os_linux.go
...
TEXT gogo<>(SB), NOSPLIT, $0
    // 将tls保存到CX寄存器
	get_tls(CX)
	// 下面这条指令把当前要运行的DX中的g，放入CX寄存器的g位置，即tls[0]这个位置，也就是线程中的本地存储
	// tls[0]保存着当前m调度的g
	MOVQ	DX, g(CX)
	MOVQ	DX, R14		// set the g register
	// 将CPU的SP寄存器设置位g.sched.sp这样就完成了栈的切换，从g0切换为g
	MOVQ	gobuf_sp(BX), SP	// restore SP
	// 将ret、ctxt、bp分别存入对应的寄存器，完成了CPU上下文的切换
	MOVQ	gobuf_ret(BX), AX
	MOVQ	gobuf_ctxt(BX), DX
	MOVQ	gobuf_bp(BX), BP
	// 清空sched的值，相关值已经存入到寄存器中，这里清空后可以减少GC的工作量
	...
	// 把sched。pc放入BX寄存器
	MOVQ	gobuf_pc(BX), BX
	// JMP 把 BX的值放入CPU的IP寄存器，此时CPU从PC处开始执行指令
	JMP	BX
...
goodm:
	...
	get_tls(CX)		// Set G in TLS,将g的信息保存在 线程本地存储
	MOVQ	R14, g(CX)
...
// goexit 用户协程完成后将调用goexit， goexit->goexit1->mcall(切换协程栈至g0)-goexit0(完成g的清理工作)
